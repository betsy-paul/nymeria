...
SCRIPT_NAME = "S12-Game_night"
ZIPS_DIR = "downloaded_s12_zips"
all_data = []
...
json_filepath = "nymeria_s12_download_urls.json"

if not os.path.exists(json_filepath):
    print(f"Error: JSON file '{json_filepath}' not found.")
    print("Please ensure 'nymeria_s12_download_urls.json' is in the script's directory or provide the full path.")
    exit()
...
for zip_file in os.listdir(ZIPS_DIR):
    ...
    subject_id = zip_file.replace(".zip", "")
    ...
    try:
        ...
        print(f"[FAIL] Error processing {zip_file}: {e}")
...
if not all_data:
    ...
    print("   1. 'nymeria_s12_download_urls.json' structure not matching expectations.")
...
# ################ ACT AVERAGE HEATMAPS #########################
for script in combined_df["script"].unique():
    # Filter for current script and drop NaNs
    ...
    # Compute average yaw and pitch per participant
    averaged_subset = (
        subset
        .groupby("subject_id")[["avg_yaw", "pitch"]]
        .mean()
        .reset_index()
    )

    averaged_subset["avg_yaw_deg"] = np.degrees(averaged_subset["avg_yaw"])
    averaged_subset["pitch_deg"] = np.degrees(averaged_subset["pitch"])


    print(f"Plotting average-of-averages heatmap for {script} with {len(averaged_subset)} participants...")
    ...
    sns.kdeplot(
        ...
        bw_adjust=1, #bandwidth to help with consistency or smth, girl read the docs
        #norm=norm
    )

    plt.title(f"Average of Averages: Yaw vs. Pitch for {script}")
    ...
    heatmap_filename = os.path.join(HEATMAP_OUTPUT_DIR, f'avg_of_avg_heatmap_{script.replace(" ", "_")}.png')
    plt.savefig(heatmap_filename)
    plt.close()
    print(f"Saved average-of-averages heatmap to {heatmap_filename}")
...
for script in combined_df["script"].unique():
    subset = combined_df[combined_df["script"] == script].dropna(subset=["avg_yaw", "pitch"])
    if subset.empty:
        print(f"Skipping '{script}' due to empty data.")
        continue
    ...
    # Plot KDE of participant-level averages
    plt.figure(figsize=(10, 8))
    sns.kdeplot(
        data=averaged_subset,
        x="avg_yaw_deg",
        y="pitch_deg",
        fill=True,
        cmap="afmhot_r",  # Use a different colormap to distinguish from density
        bw_adjust=0.5,   # Optional: tune for consistency
        thresh=0.05,
        levels=50,
        alpha=0.7,
        cbar=True,
        cbar_kws={'label': 'Participant Average Density'}
    )
    plt.title(f"Participant-Level KDE: Yaw vs. Pitch for {script}")
    plt.xlabel("Average Yaw (degrees)")
    plt.ylabel("Average Pitch (degrees)")
    plt.xlim(-10, 10)
    plt.ylim(-35, -0)
    ...
    variance_filename = os.path.join(
        HEATMAP_OUTPUT_DIR,
        f'variance_kde_{script.replace(" ", "_")}.png'
    )
    plt.savefig(variance_filename)
    plt.close()
    print(f"Saved variance KDE heatmap to {variance_filename}")

############ AVG OF AVGs HEATMAPS with np and pandas ##########
...
#     # count_matrix[(12, 33)] = { #example of what the inner code is doing bc ik you're gonna forget how we got here
#     #     "amanda": 2,
#     #     "kenneth": 5,
#     #     "seth": 1
#     # }
...
#         if len(counts) > 1:
#             grid[j, i] = np.log(np.average(counts) + 0.001)
#         else:
...
#     plt.colorbar(label="Average of Averages Heatmaps")
#     plt.title(f"Average of Averages Heatmap of Gaze: {script}")
#     plt.xlabel("Average Yaw (radians)")
#     plt.ylabel("Pitch (radians)")
#     plt.grid(True, linestyle='--', alpha=0.5)
#     plt.tight_layout()

#     outpath = os.path.join(HEATMAP_OUTPUT_DIR, f"avg_averages_map_{script.replace(' ', '_')}.png")
#     plt.savefig(outpath)
#     plt.close()
#     print(f"📈 Saved average of averages map for {script} → {outpath}")
